...
def _csv_row_to_db_dict(row: dict) -> dict:
    ...
    def _datetime_or_none(val: str | None) -> str | None:
        """Normalize to iso-formatted datetime strings, consistent with API import"""
        dt = try_datetime(val) if val else None
        return dt.isoformat() if dt else (val or None)

    # CSV exports don't provide a total identifications count, only current agreement/disagreement
    # counts, so this is an estimate (excludes withdrawn/non-current identifications)
    ...
    return {
        ...
        'observed_on': _datetime_or_none(row.get('observed_on')),
